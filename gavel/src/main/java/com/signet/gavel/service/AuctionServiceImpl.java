...
public class AuctionServiceImpl extends AuctionServiceGrpc.AuctionServiceImplBase {
        ...
        public void placeBid(BidRequest request, StreamObserver<BidResponse> responseObserver) {
                try {
                        ...
                        if (auction.getCreatorId() != null && auction.getCreatorId().equals(request.getUserId())) {
                                responseObserver.onNext(BidResponse.newBuilder()
                                                .setSuccess(false)
                                                .setMessage("You cannot bid on your own auction.")
                                                .setCurrentPrice(auction.getCurrentPrice())
                                                .build());
                                responseObserver.onCompleted();
                                return;
                ...
                auctionRepository.findById(request.getId()).ifPresentOrElse(
                                auction -> {
                                        responseObserver.onNext(AuctionState.newBuilder()
                                                        ...
                                                        .setCreatorId(auction.getCreatorId() == null ? ""
        ...
        public void listAuctions(ListAuctionsRequest request, StreamObserver<ListAuctionsResponse> responseObserver) {
                List<AuctionState> states = auctionRepository.findAll().stream()
                                .map(auction -> AuctionState.newBuilder()
                                                ...
                                                .setItem(auction.getItem())
                                                .setCurrentPrice(auction.getCurrentPrice())
                                                .setHighestBidderId(auction.getHighestBidderId() == null ? ""
                                                                : auction.getHighestBidderId())
                                                .setCreatorId(auction.getCreatorId() == null ? ""
                                                                : auction.getCreatorId())
                                                .build())
        ...
        public void createAuction(CreateAuctionRequest request,
                ...
                try {
                        if (auctionRepository.existsById(request.getId())) {
                                responseObserver.onNext(CreateAuctionResponse.newBuilder()
                                                .setSuccess(false)
                                                .setMessage("An auction with ID '" + request.getId()
                                                                + "' already exists.")
                                                .build());
                                responseObserver.onCompleted();
                                return;
                        ...
                        auction.setItem(request.getItem());
                        auction.setCurrentPrice(request.getStartingPrice());
                        auction.setCreatorId(request.getCreatorId());
                        auctionRepository.save(auction);
